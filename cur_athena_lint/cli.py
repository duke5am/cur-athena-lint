...
# Paths are relative to THIS file's directory. The SQL ships INSIDE the package
# (cur_athena_lint/queries/ and cur_athena_lint/setup/), so HERE is the content
# root both in an installed wheel and in a checkout. If a content/ directory is
# present (the full pack layout) it is preferred. Set CUR_CONTENT to point the
# checker at a different tree, e.g. when vendoring the queries elsewhere.
CONTENT = os.environ.get('CUR_CONTENT', os.path.join(HERE, 'content'))
PACK_MODE = os.path.isdir(CONTENT)
if not PACK_MODE:
    CONTENT = HERE
# PACK_MODE is False for the standalone layout, where there is no
...
#
# Directories that are never shipped content. Walking them made P13 report git
# objects as "not UTF-8" and short metadata files as "non-trivial", so the
# checker FAILED (exit 1) inside its own repository, which is the one place it is
# most likely to be run.
SKIP_DIRS = {'.git', '.hg', '.svn', '.tox', '.venv', '__pycache__', 'build', 'dist'}
...
def _run():
    """Run every structural check and return an exit code (0 pass, 1 fail).

    The body below is the original top-level script, unchanged, wrapped in a
    function so the same code can be the `cur-athena-lint` console script.
    """
    results = []
    ...
    # ---------------------------------------------------------------------------
    # An explicit CUR_CONTENT that does not exist must NOT fall back to the bundled
    # pack. Checking a different tree than the one asked for, and exiting 0 with
    # "all checks passed", is the worst possible answer.
    override = os.environ.get('CUR_CONTENT')
    if override and not os.path.isdir(override):
        sys.stderr.write(
            "error: CUR_CONTENT is set to %r, which is not a directory - nothing "
            "was checked.\n" % override)
        return 2
    ...
    def read_sql(path):
        """Return the file's text, or None if it cannot be read as UTF-8 text.

        Every check below used to re-open the .sql files with no guard, so one
        unreadable file (a latin-1 comment, a binary file with a .sql name) raised
        UnicodeDecodeError out of the middle of the run: the user got a traceback
        naming no file, and every remaining check was skipped. P13 reports the
        unreadable files; the other checks just skip them.
        """
        try:
            return open(path, encoding='utf-8').read()
        except (OSError, UnicodeDecodeError) as exc:
            unreadable.setdefault(os.path.basename(path),
    ...
    for f in files:
        ...
        if src is None:
            parse_failures.append(
                (name, "could not read: " + unreadable[os.path.basename(f)]))
            continue
        try:
    ...
    for f in glob.glob(os.path.join(QDIR, '*.sql')):
        ...
        if q_src is None:
            continue
        for stmt in sqlglot.parse(q_src, read='athena'):
    ...
    for f in glob.glob(os.path.join(QDIR, '*.sql')):
        ...
        src = read_sql(f)
        if src is None:
            missing_headers.append(f"{name}: could not be read as UTF-8 text")
            continue
    ...
    for f in glob.glob(os.path.join(QDIR, '*.sql')):
        ...
        src = read_sql(f)
        if src is None:
            no_warning.append(f"{name}: could not be read as UTF-8 text")
            continue
    ...
    # Leading whitespace is allowed: the checks below are indented inside _run(),
    # and an anchored pattern without \s* silently measured 0 of them.
    STATIC_CHECK_COUNT = len(re.findall(r"^\s*check\(", SOURCE_TEXT, re.M))
...
def main(argv=None):
    """Console-script entry point: a readable one-line error, never a traceback."""
    try:
        return _run()
